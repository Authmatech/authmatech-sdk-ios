...
final class TraceCollector {
    ...
    let queue = DispatchQueue(label: "com.authmatech.tracecollector.queue")
    ...
    func addDebug(type: OSLogType = .debug, tag: String? = "Authmatech", log: String) {
        queue.async {
...
public class DebugInfo {
    ...
    public func userAgent(sdkVersion: String) -> String {
        return "authmatech-sdk-ios/\(sdkVersion) \(deviceString())"
    }
    ...
    func addDebug(tag: String = "Authmatech", traceId: String? = nil, log: String) {
        let now = self.dateUtils.now()
        let composedLog = traceId != nil ? "\(tag):[\(traceId!)] - \(log)" : "\(tag) - \(log)"
        self.bufferMap[now] = composedLog
    }
